...
export default function DayPlanSidebar({
  ...
  const getMergedItems = (dayId) => {
    ...
    // If any place has a time, sort by place_time (minutes). Otherwise use order_index.
    const hasTimes = da.some(a => parseTimeToMinutes(a.place?.place_time) !== null)

    if (hasTimes) {
      // sortKey for a place: its time in minutes (0-1439) or 1440+order_index for untimed places
      const placeKey = (a: any): number => {
        const min = parseTimeToMinutes(a.place?.place_time)
        return min !== null ? min : 1440 + a.order_index
      }
      // sortKey for notes: interpolate between the time positions of their adjacent places
      const noteKey = (n: any): number => {
        const before = Math.floor(n.sort_order)
        const after = Math.ceil(n.sort_order)
        const beforePlace = da.find(a => a.order_index === before)
        const afterPlace = da.find(a => a.order_index === after)
        const bKey = beforePlace !== undefined ? placeKey(beforePlace) : -1
        const aKey = afterPlace !== undefined ? placeKey(afterPlace) : 1440 + da.length
        return (bKey + aKey) / 2
      }
      return [
        ...da.map(a => ({ type: 'place' as const, sortKey: placeKey(a), data: a })),
        ...dn.map(n => ({ type: 'note' as const, sortKey: noteKey(n), data: n })),
        ...transport.map(r => ({
          type: 'transport' as const,
          sortKey: parseTimeToMinutes(r.reservation_time) ?? (r.day_plan_position ?? computeTransportPosition(r, da)),
          data: r,
        })),
