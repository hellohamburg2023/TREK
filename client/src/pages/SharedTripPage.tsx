...
import { Clock, MapPin, FileText, Train, Plane, Bus, Car, Ship, Ticket, Hotel, Map, Luggage, Wallet, MessageCircle, Receipt } from 'lucide-react'
        ...
        {activeTab === 'kosten' && kostenData && (() => {
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {/* Total */}
              <div style={{ background: 'linear-gradient(135deg, #000 0%, #1a1a2e 100%)', borderRadius: 14, padding: '20px 24px', color: 'white' }}>
                <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: 1, textTransform: 'uppercase', opacity: 0.5 }}>Kosten gesamt</div>
                <div style={{ fontSize: 28, fontWeight: 700, marginTop: 4 }}>{fmtAmt(total)} {currency}</div>
                <div style={{ fontSize: 11, opacity: 0.4, marginTop: 2 }}>{expenses.length} Ausgabe{expenses.length !== 1 ? 'n' : ''}</div>
              </div>
              ...
              {balances.length > 0 && (
                <div style={{ background: 'var(--bg-card, white)', borderRadius: 12, border: '1px solid var(--border-faint, #e5e7eb)', overflow: 'hidden' }}>
                  <div style={{ padding: '10px 16px', background: '#f9fafb', borderBottom: '1px solid #f3f4f6' }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#374151' }}>Bilanz pro Person</span>
                  </div>
                    ...
                    <div key={i} style={{ padding: '9px 16px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: i < balances.length - 1 ? '1px solid #fafafa' : 'none' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 1 }}>
                          bezahlt {fmtAmt(b.paid)} · Anteil {fmtAmt(b.owed)}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {debts.length > 0 && (
                <div style={{ background: 'var(--bg-card, white)', borderRadius: 12, border: '1px solid var(--border-faint, #e5e7eb)', overflow: 'hidden' }}>
                  <div style={{ padding: '10px 16px', background: '#f9fafb', borderBottom: '1px solid #f3f4f6' }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#374151' }}>Wer zahlt wem?</span>
                  </div>
                    ...
                    <div style={{ padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: expShares.length > 0 ? '1px solid #f3f4f6' : 'none' }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>
                          ...
                          {paidByUser ? <span>von {paidByUser.username}</span> : e.paid_by_name ? <span>von {e.paid_by_name}</span> : null}
